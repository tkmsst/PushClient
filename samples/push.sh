...
while read token; do
	for i in `seq $cnt`; do
		RESULT=`curl -k --header "Authorization: key=$APIKEY" --header Content-Type:"application/json" https://fcm.googleapis.com/fcm/send -d "{\"to\":\"$token\",\"collapse_key\":\"pushclient\",\"priority\":\"high\",\"content_available\":true,\"data\":{\"app\":\"$APPLICATION\",\"title\":\"$TITLE\",\"msg\":\"$MESSAGE\"}}"`
